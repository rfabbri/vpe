...
"        Mods: Ricardo Fabbri Mon Jun 25 20:15:00 EDT 2007 
"                 - added support for table of symbols (nomencl.sty) compilation
" ============================================================================
...
function! Tex_CompileMultipleTimes()
	...
	let nomidxFileName = mainFileName_root.'.nlo'
	...
	while needToRerun == 1 && runCount < 5
		...
		let nomidxlinesBefore = Tex_CatFile(nomidxFileName)
		...
		let nomidxlinesAfter = Tex_CatFile(nomidxFileName)
    ...
    " XXX rfabbri
		...
		" file and remember to rerun latex.
		if runCount == 0 && glob(nomidxFileName) != '' && nomidxlinesBefore != nomidxlinesAfter
			echomsg "Running makeindex for nomenclature..."
			let temp_mp = &mp | let &mp='makeindex $*.nlo -s nomencl.ist -o $*.nls'
			exec 'silent! make '.mainFileName_root
			let &mp = temp_mp
		...
		if runCount == 0 && Tex_IsPresentInFile('\\bibdata', mainFileName_root.'.aux')
			...
			let temp_mp = &mp | let &mp='bibtex --min-crossrefs=1000'
			exec 'silent! make '.mainFileName_root
			let &mp = temp_mp
